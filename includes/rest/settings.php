...
function blueworx_headless_register_settings_page() {
	add_submenu_page(
		...
		esc_html__( 'Headless', 'blueworx-project-wordpress-labs' ),
		esc_html__( 'Headless', 'blueworx-project-wordpress-labs' ),
		'manage_options',
		'blueworx-headless',
...
function blueworx_headless_save_settings() {
	...
	set_transient( 'blueworx_headless_notice', __( 'Headless settings saved.', 'blueworx-project-wordpress-labs' ), 30 );
...
function blueworx_headless_generate_invite() {
	...
	set_transient( 'blueworx_headless_notice', __( 'Invite created. Copy the token below now — it is shown only once.', 'blueworx-project-wordpress-labs' ), 60 );
...
function blueworx_headless_render_constant_status( $label, $is_set ) {
	...
	<tr>
		...
		<td>
			<strong>
				<?php echo $is_set ? esc_html__( 'Configured', 'blueworx-project-wordpress-labs' ) : esc_html__( 'Not set', 'blueworx-project-wordpress-labs' ); ?>
			</strong>
		</td>
	</tr>
	<?php
	...
	<div class="wrap">
		...
		<?php if ( $new_invite ) : ?>
			<div class="notice notice-info">
				<p><?php esc_html_e( 'New invite token (shown once):', 'blueworx-project-wordpress-labs' ); ?></p>
				<p><input type="text" class="large-text code" readonly value="<?php echo esc_attr( $new_invite ); ?>" onclick="this.select();" /></p>
			</div>
		<?php endif; ?>

		<h2><?php esc_html_e( 'Secrets (wp-config.php constants)', 'blueworx-project-wordpress-labs' ); ?></h2>
		<p class="description"><?php esc_html_e( 'Secrets are read only from wp-config.php and never stored in the database.', 'blueworx-project-wordpress-labs' ); ?></p>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<h2><?php esc_html_e( 'Accounts', 'blueworx-project-wordpress-labs' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="registration_mode"><?php esc_html_e( 'Registration mode', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Email verification', 'blueworx-project-wordpress-labs' ); ?></th>
					<td><label><input type="checkbox" name="email_verification_required" value="1" <?php checked( '1', blueworx_headless_setting( 'email_verification_required' ) ); ?> /> <?php esc_html_e( 'Require email confirmation before sign-in', 'blueworx-project-wordpress-labs' ); ?></label></td>
				</tr>
				<tr>
					<th scope="row"><label for="default_role"><?php esc_html_e( 'Default role', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td>
			...
			<h2><?php esc_html_e( 'Tokens & lockout', 'blueworx-project-wordpress-labs' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="access_ttl"><?php esc_html_e( 'Access token lifetime (seconds)', 'blueworx-project-wordpress-labs' ); ?></label></th>
				...
				<tr>
					<th scope="row"><label for="refresh_ttl_days"><?php esc_html_e( 'Refresh token lifetime (days)', 'blueworx-project-wordpress-labs' ); ?></label></th>
				...
				<tr>
					<th scope="row"><label for="login_max_attempts"><?php esc_html_e( 'Login attempts before lockout', 'blueworx-project-wordpress-labs' ); ?></label></th>
				...
				<tr>
					<th scope="row"><label for="login_window"><?php esc_html_e( 'Lockout window (seconds)', 'blueworx-project-wordpress-labs' ); ?></label></th>
			...
			<h2><?php esc_html_e( 'Frontend & CORS', 'blueworx-project-wordpress-labs' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="frontend_url"><?php esc_html_e( 'Frontend URL', 'blueworx-project-wordpress-labs' ); ?></label></th>
				...
				<tr>
					<th scope="row"><label for="allowed_origins"><?php esc_html_e( 'Allowed origins', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'One origin per line or comma-separated. Overridden by the BLUEWORX_LABS_ALLOWED_ORIGINS constant when set.', 'blueworx-project-wordpress-labs' ); ?></p>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Revalidation & integrations', 'blueworx-project-wordpress-labs' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Revalidation webhook', 'blueworx-project-wordpress-labs' ); ?></th>
					<td><label><input type="checkbox" name="revalidate_enabled" value="1" <?php checked( '1', blueworx_headless_setting( 'revalidate_enabled' ) ); ?> /> <?php esc_html_e( 'Ping the frontend to revalidate changed content (never triggers a full build)', 'blueworx-project-wordpress-labs' ); ?></label></td>
				</tr>
				<tr>
					<th scope="row"><label for="revalidate_url"><?php esc_html_e( 'Revalidation URL', 'blueworx-project-wordpress-labs' ); ?></label></th>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'SureCart proxy', 'blueworx-project-wordpress-labs' ); ?></th>
					<td><label><input type="checkbox" name="surecart_enabled" value="1" <?php checked( '1', blueworx_headless_setting( 'surecart_enabled' ) ); ?> /> <?php esc_html_e( 'Enable the SureCart proxy (requires the API key constant)', 'blueworx-project-wordpress-labs' ); ?></label></td>
				</tr>
				<tr>
					<th scope="row"><label for="cpts"><?php esc_html_e( 'Custom post types in REST', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Comma-separated post-type keys to expose on the core REST API.', 'blueworx-project-wordpress-labs' ); ?></p>
					</td>
				</tr>
			</table>

			<?php submit_button( esc_html__( 'Save Headless Settings', 'blueworx-project-wordpress-labs' ) ); ?>
		</form>

		<h2><?php esc_html_e( 'Create invite', 'blueworx-project-wordpress-labs' ); ?></h2>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="invite_email"><?php esc_html_e( 'Pin to email (optional)', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td><input type="email" name="invite_email" id="invite_email" class="regular-text" /></td>
				</tr>
				<tr>
					<th scope="row"><label for="invite_role"><?php esc_html_e( 'Role (optional)', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td>
						<select name="invite_role" id="invite_role">
							<option value=""><?php esc_html_e( '— Default —', 'blueworx-project-wordpress-labs' ); ?></option>
							<?php foreach ( $roles as $slug => $name ) : ?>
				...
				<tr>
					<th scope="row"><label for="invite_days"><?php esc_html_e( 'Expires in (days)', 'blueworx-project-wordpress-labs' ); ?></label></th>
					<td><input type="number" name="invite_days" id="invite_days" min="1" value="7" class="small-text" /></td>
				</tr>
			</table>
			<?php submit_button( esc_html__( 'Create Invite', 'blueworx-project-wordpress-labs' ), 'secondary' ); ?>
		</form>
	</div>
	<?php
}
